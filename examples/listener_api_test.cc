...
int main() {
  ...
  int listener_id = client.get_connection()->add_listener(
      "test-bucket", "*", true, [&client](const Event& event) {
  ...
  int listener_id2 = client.get_connection()->add_listener(
      "test-bucket", "specific-key", true,
      [](const Event& event) { std::cout << "Specific key event: " << event.key << std::endl; });
