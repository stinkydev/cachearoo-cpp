...
void Requestor::CheckTimeouts() {
  while (!shutdown_.load()) {
    ...
    for (auto it = pending_requests_.begin(); it != pending_requests_.end();) {
      auto progress_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
          now - it->second->last_progress).count();
      auto total_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
          now - it->second->request_time).count();
...
void Replier::OnRequest(const Event& event) {
  ...
  MessageResponseCallback response_callback = [this, key = event.key](
      const std::string& error, const std::string& data) {
  ...
  while (!shutdown_.load()) {
    ...
    for (auto it = pending_jobs_.begin(); it != pending_jobs_.end();) {
      auto progress_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
          now - it->second->last_progress).count();
      auto total_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
          now - it->second->publish_time).count();
  ...
  try {
    ...
    if (work_handler) {
      MessageResponseCallback callback = [this, key, status_obj](
          const std::string& error, const std::string& data) mutable {
        job_count_--;
      ...
      ProgressCallback progress_callback = [this, key, status_obj](
          const std::string& progress) mutable {
