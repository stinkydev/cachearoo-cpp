...
Requestor::Requestor(CachearooClient* client, const std::string& channel,
  ...
  client_->GetConnection()->AddListener(progress_channel_, "*", true,
                                        progress_listener_);
...
std::future<std::string> Requestor::RequestAsync(
    const std::string& message, ProgressCallback progress_callback, int timeout,
    int progress_timeout) {
  if (timeout == 0) timeout = timeout_;
  if (progress_timeout == 0) progress_timeout = progress_timeout_;
...
std::string Requestor::Request(const std::string& message,
                               ProgressCallback progress_callback, int timeout,
                               int progress_timeout) {
  return RequestAsync(message, progress_callback, timeout, progress_timeout)
      .get();
}
  ...
  while (!shutdown_.load()) {
    ...
    for (auto it = pending_requests_.begin(); it != pending_requests_.end();) {
      ...
      if (is_progress_timeout || is_timeout) {
        it->second->promise.set_exception(std::make_exception_ptr(TimeoutError(
            "Request timeout for id " + it->first, is_progress_timeout)));
...
Replier::Replier(CachearooClient* client, const std::string& channel)
    : client_(client) {
  channel_ = std::string(kRequestReplyPrefix) + channel;
  progress_channel_ = std::string(kProgressPrefix) + channel;
...
void Replier::OnRequest(const Event& event) {
  if (!message_handler_ || !event.value) return;

  MessageResponseCallback response_callback = [this, key = event.key](
  ...
  ProgressCallback progress_callback =
      [this, key = event.key](const std::string& data) {
        nlohmann::json progress;
        progress["data"] = data;
        client_->GetConnection()->SignalEvent(progress_channel_, key,
                                              progress.dump());
      };
...
Producer::Producer(CachearooClient* client, const std::string& channel,
                   int timeout, int progress_timeout)
    : client_(client), timeout_(timeout), progress_timeout_(progress_timeout) {
  ...
  client_->GetConnection()->AddListener(status_queue_, "*", true,
                                        job_status_listener_);
...
std::future<std::string> Producer::AddJobAsync(
    const std::string& job, ProgressCallback progress_callback, int timeout,
    int progress_timeout) {
  if (timeout == 0) timeout = timeout_;
  if (progress_timeout == 0) progress_timeout = progress_timeout_;
  ...
  auto now = std::chrono::steady_clock::now();
  pending_job->last_progress =
      now;  // Will be set to null initially, but simplified here
...
std::string Producer::AddJob(const std::string& job,
                             ProgressCallback progress_callback, int timeout,
                             int progress_timeout) {
...
void Producer::CheckTimeouts() {
  while (!shutdown_.load()) {
    ...
    for (auto it = pending_jobs_.begin(); it != pending_jobs_.end();) {
      ...
      if (is_progress_timeout || is_timeout) {
        it->second->promise.set_exception(std::make_exception_ptr(TimeoutError(
            "Job timeout for id " + it->first, is_progress_timeout)));
...
CompetingConsumer::CompetingConsumer(CachearooClient* client,
  ...
  client_->GetConnection()->AddListener(job_queue_, "*", true,
                                        job_queue_listener_);
}

CompetingConsumer::~CompetingConsumer() { Destroy(); }
...
void CompetingConsumer::OnJobReceived(const Event& event) {
  if (event.is_deleted || !event.value) return;
  ...
  try {
    ...
    std::thread([this, key = event.key, job]() {
      JobHandler(key, job);
    }).detach();
...
void CompetingConsumer::JobHandler(const std::string& key,
  ...
  try {
    ...
    if (work_handler) {
      MessageResponseCallback callback = [this, key, status_obj](
                                             const std::string& error,
      ...
      ProgressCallback progress_callback =
          [this, key, status_obj](const std::string& progress) mutable {
            status_obj["progress"] = progress;
            client_->GetConnection()->Write(status_queue_, key,
                                            status_obj.dump());
...
std::shared_ptr<Worker> CompetingConsumer::WaitForWorker(
    const nlohmann::json& job) {
  if (!job_query_handler_) return nullptr;
  ...
  while (!worker_found) {
    ...
    job_query_handler_(job.dump(),
                       [&](int error_code, std::shared_ptr<Worker> worker) {
