...
CachearooConnection::CachearooConnection(const CachearooSettings& settings)
  ...
  ws_client_->set_message_handler([this](ConnectionHdl hdl, WebSocketClient::message_ptr msg) {
    OnMessage(hdl, msg);
  });
...
void CachearooConnection::Close() {
  ...
  for (auto& [id, request] : pending_requests_) {
    request->promise.set_exception(std::make_exception_ptr(std::runtime_error("Connection closed")));
  }
...
void CachearooConnection::OnOpen(ConnectionHdl hdl) {
  ...
  
  {
    ...
    while (!pending_message_queue_.empty()) {
      try {
        ws_client_->send(connection_hdl_, pending_message_queue_.front(), websocketpp::frame::opcode::text);
      } catch (const std::exception& e) {
        if (on_error_) on_error_(std::string("WebSocket send failed (OnOpen): ") + e.what());
      }
...
void CachearooConnection::ProcessStringMessage(const std::string& message) {
  try {
    ...
    if (on_error_) on_error_("Failed to process message: " + std::string(e.what()));
  }
}
...
void CachearooConnection::AddRequest(const nlohmann::json& request, std::promise<std::string>& promise) {
  ...
  
  {
    ...
    if (connected_.load() && ws_client_) {
      try {
        ...
        if (on_error_) on_error_(std::string("WebSocket send failed: ") + e.what());
        std::lock_guard<std::mutex> lock2(requests_mutex_);
        auto it = pending_requests_.find(id);
        if (it != pending_requests_.end()) {
          it->second->promise.set_exception(std::make_exception_ptr(std::runtime_error(std::string("WebSocket send failed: ") + e.what())));
        }
      }
    } else {
